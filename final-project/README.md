...
```bash
curl.exe -X POST http://localhost:3000/api/users -H "Content-Type: application/json" -d '{"email":"John","phone":"phdisqnjds", "agreement":false}'
```
...
```bash
curl -X POST http://localhost:8080/api/users -H "Content-Type: application/json" -d '{"email":"lisa@doe.com","phone":"+11111111","agreement":"false"}'
```
---
